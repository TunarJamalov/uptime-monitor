...
export async function notify(provider: string | undefined, webhook: string | undefined, event: 'DOWN' | 'STILL_DOWN' | 'RECOVERED' | 'SLOW' | 'SSL_EXPIRING' | 'DOMAIN_EXPIRING', monitor: MonitorRow, details: string) {
  ...
  if (provider === 'generic' || provider === 'webhook') payload = { event, monitor: { id: monitor.id, name: monitor.name, type: monitor.type, url: monitor.url }, status: event === 'RECOVERED' ? 'UP' : event === 'DOWN' || event === 'STILL_DOWN' ? 'DOWN' : event, timestamp: new Date().toISOString(), message: details };
