...
export async function notify(provider: string | undefined, webhook: string | undefined, event: 'DOWN' | 'RECOVERED' | 'SLOW' | 'SSL_EXPIRING', monitor: MonitorRow, details: string) {
  if (!webhook) return;
  let payload: unknown;
  const title = event === 'DOWN' ? `DOWN: ${monitor.name}` : event === 'RECOVERED' ? `RECOVERED: ${monitor.name}` : event === 'SLOW' ? `SLOW RESPONSE: ${monitor.name}` : `SSL EXPIRING: ${monitor.name}`;
...
export async function emailNotify(event: 'DOWN' | 'RECOVERED' | 'SLOW' | 'SSL_EXPIRING', monitor: MonitorRow, details: string) {
