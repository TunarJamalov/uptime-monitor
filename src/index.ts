...
import { backupDatabase, cleanup, getMonitor, listMonitors, monitorCount, openDatabase, recordCheck, syncMonitors } from './db.js';
...
function startMonitor(monitor: MonitorConfig) { stopMonitor(monitor.id); if (!monitor.active || getMonitor(db,monitor.id)?.maintenance) return; void runMonitor(monitor); timers.set(monitor.id,setInterval(()=>void runMonitor(monitor),monitor.interval*1000)); }
...
refreshSchedules(); const cleanupTimer=setInterval(()=>{ cleanup(db); if (process.env.BACKUP_DIR) void backupDatabase(db,process.env.BACKUP_DIR).catch(error=>console.error(`Database backup failed: ${error instanceof Error ? error.message : error}`)); },24*60*60*1000); cleanup(db);
const app=createServer(db,refreshSchedules,runMonitor); const port=Number(process.env.PORT ?? 3000); const server=app.listen(port,()=>console.log(`Uptime monitor listening on port ${port}`));
function shutdown() { timers.forEach(clearInterval); clearInterval(cleanupTimer); server.close(()=>{db.close();process.exit(0);}); }
