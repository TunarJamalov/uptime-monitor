...
import { backupDatabase, cleanup, evaluateHeartbeats, getMonitor, listMonitors, logNotification, markIncidentNotified, monitorCount, openDatabase, recordCheck, recordDomainResult, repeatIncidents, syncMonitors } from './db.js';
...
async function deliver(monitor:any,event:'DOWN'|'STILL_DOWN'|'RECOVERED'|'SLOW'|'SSL_EXPIRING'|'DOMAIN_EXPIRING',details:string,incidentId?:number) { if(monitor.notificationsEnabled===false || (event==='RECOVERED'&&monitor.recoveryNotifications===false)) return; const [webhook,email]=await Promise.all([notify(process.env.WEBHOOK_PROVIDER,process.env.WEBHOOK_URL,event,monitor,details),emailNotify(event,monitor,details)]); logNotification(db,monitor.id,incidentId,event,process.env.WEBHOOK_PROVIDER??'webhook',webhook.success,webhook.error); logNotification(db,monitor.id,incidentId,event,'smtp',email.success,email.error); if(incidentId&&(event==='DOWN'||event==='STILL_DOWN'))markIncidentNotified(db,incidentId); }
