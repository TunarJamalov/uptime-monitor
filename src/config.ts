...
export function validateMonitor(raw: unknown, id: string, label = 'Monitor'): MonitorConfig {
  ...
  if (type === 'heartbeat') {
    if (m.expectedInterval !== undefined && m.expectedInterval !== 0 && (typeof m.expectedInterval !== 'number' || !Number.isFinite(m.expectedInterval) || m.expectedInterval < 1)) throw new Error(`${label}: expectedInterval must be at least 1 second`);
