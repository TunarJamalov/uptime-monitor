...
export function validateMonitor(raw: unknown, id: string, label = 'Monitor'): MonitorConfig {
  ...
  if (m.failureThreshold !== undefined && (!Number.isInteger(m.failureThreshold) || Number(m.failureThreshold) < 1)) throw new Error(`${label}: failureThreshold must be a positive integer`);
  if (m.repeatNotificationMinutes !== undefined && (!Number.isFinite(Number(m.repeatNotificationMinutes)) || Number(m.repeatNotificationMinutes) < 0)) throw new Error(`${label}: repeatNotificationMinutes must be zero or greater`);
  for (const key of ['recoveryNotifications','notificationsEnabled']) if (m[key] !== undefined && typeof m[key] !== 'boolean') throw new Error(`${label}: ${key} must be boolean`);
